...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def connectDatabase(self):
        ...
        self.__engine = sqlalchemy.create_engine(
            "mysql://juliagod_readuser:labtests-heliohost@johnny.heliohost.org:3306/juliagod_labtests", pool_recycle=280)
    ...
    def saveResultsToDict(self, tabGuiChoices, gender, age, typeAge, dictType):
        ...
        for tested_aim, desc in tabGuiChoices.items():
            for (shortname, resultAmount, unitname) in desc:
                targetfullname = self.__connection.execute(
                    "SELECT `targetfullname` FROM `targetdesc` WHERE `targetshortname` = '{0}'".format(shortname)).fetchone()
                statement = self.__connection.execute(self.getDataQueryText(
                    targetfullname['targetfullname'], unitname, typeAge, age, resultAmount, gender)).fetchone()
