...
class TestDB(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.__engine = sqlalchemy.create_engine(
            "mysql://juliagod_readuser:labtests-heliohost@johnny.heliohost.org:3306/juliagod_labtests", pool_recycle=280)  # of course it's not real yet
    ...
    def test_containNotEqualTargets(self):
        ...
        statement = self.__connection.execute(
            "SELECT * FROM `targetdesc` WHERE `targetfullname` NOT IN(SELECT `labtests`.`targetfullname` FROM `labtests`)").fetchall()

        if statement:
            for row_number, row in enumerate(statement):
                print("In targetdesc exists other value: ",
                      row["targetfullname"], " at row ", row_number)
    ...
    def test_containNotEqualTargetsLabTests(self):
        ...
        statement = self.__connection.execute(
            "SELECT * FROM `labtests` WHERE `targetfullname` NOT IN(SELECT `targetdesc`.`targetfullname` FROM `targetdesc`)").fetchall()

        if statement:
            for row_number, row in enumerate(statement):
                print("In labtests exists other value: ",
                      row["targetfullname"], " at row ", row_number)
    ...
    def test_containsDuplications(self):
        ...
        statement = self.__connection.execute(
            "SELECT `gender`, `age-range-start`, `age-range-end`, `age-type`, `targetfullname`, count(*) FROM `labtests` GROUP BY `gender`, `age-range-start`, `age-range-end`, `age-type`, `targetfullname` HAVING count(*) > 1").fetchall()
